        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex-1 space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {selectedFiles.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-100 bg-slate-50/30 p-10 text-center dark:border-slate-800 dark:bg-slate-900/30">
                  <div className="mb-4 rounded-2xl bg-white p-4 text-slate-300 shadow-sm dark:bg-slate-800 dark:text-slate-600 dark:border dark:border-slate-700">
                    <svg className="size-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                  </div>
                  <p className="text-sm text-slate-400 dark:text-slate-500">No files selected yet</p>
                </div>
              ) : (
                selectedFiles.map((file, index) => (
                  <div
                    key={`${file.name}-${file.size}-${index}`}
                    className="flex items-center justify-between rounded-2xl border border-slate-50 bg-white p-3 shadow-sm transition-all hover:border-indigo-100 hover:bg-slate-50 group dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700/50"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`flex size-10 items-center justify-center rounded-xl ${file.name.endsWith('.pdf') ? 'bg-rose-50 dark:bg-rose-900/20 text-rose-500' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-500'}`}>
